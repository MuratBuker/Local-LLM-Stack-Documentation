...
def netbox_get_objects(object_type: str, filters: dict | str):
    ...
    """
    # Convert string to dict (with error handling)
    ...
    # Check for and handle special filter cases
    if object_type == "devices" and "tenant" in filters:
        # Get the tenant name from the filters
        tenant_name = filters["tenant"]
        
        # Look up the tenant by name to get its slug
        try:
            tenant_results = netbox.get("tenancy/tenants/", params={"name": tenant_name})
            
            # Check if a tenant was found
            if tenant_results and tenant_results.get("results"):
                # Use the slug from the first result
                tenant_slug = tenant_results["results"][0]["slug"]
                # Replace the tenant name filter with the slug filter
                filters["tenant_slug"] = tenant_slug
                del filters["tenant"]
            else:
                # Handle case where no tenant is found with that name
                raise ValueError(f"Tenant '{tenant_name}' not found.")
        except Exception as e:
            # Pass through the error from the Netbox API
            raise ValueError(f"Error resolving tenant name: {e}")


    if object_type == "device_types":
        object_type = "device-types"
    if "device_manufacturer" in filters:
        filters["manufacturer"] = filters.pop("device_manufacturer")
...
def netbox_get_object_by_id(object_type: str, object_id: int):
    ...
    """
    # Normalize object_type: "dcim.devices" → "devices"
    if "." in object_type:
        object_type = object_type.split(".", 1)[1]
    if object_type not in NETBOX_OBJECT_TYPES:
        valid_types = "\n".join(f"- {t}" for t in sorted(NETBOX_OBJECT_TYPES.keys()))
